...
class Parser {
	public:
		 ...
		 */
		Parser(std::string configFile, char** envp = 0);
		...
		inline Parser* group(std::string name) {
			return groups[name];
		}

		// get config sub group map (e.g. for iterating over all groups)
		inline std::map<std::string, Parser*>& getGroups() {
			return groups;
		}

	private:
		// private constructor for sub groups
		Parser(std::string name, std::string parentDebugInfo);
