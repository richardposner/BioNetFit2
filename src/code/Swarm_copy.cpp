...
//	}
	...
	runningSubParticles_.insert(subParID); //razi: was //
...
/*
	...
	*/
		...
		cout << "RAQUEL saving swarm" << endl;
		// Save swarm state
		saveSwarmState();
		cout << "RAQUEL done saving swarm" << endl;


		if (!stopCriteria) {
			cout << "RAQUEL saving summary" << endl;

			string outputPath = options.jobOutputDir + toString(currentGeneration) + "_summary.txt";
			cout << "RAQUEL generation "  << toString(currentGeneration) << endl;
			outputRunSummary(outputPath);
			//++currentGeneration;
			cout << "RAQUEL done saving summary" << endl;

			// Process particles
			cout << "Raquel before processParticlesPSO" << endl;

			for(int i=1; i <= numFinishedParticles; i++){
				particlestoProcess.push_back(i);
			}

			path = options.jobOutputDir + toString(currentGeneration);
